...
const mapToName = (resp) => {
	return resp.map(author => ({ label: ` ${author.profile.firstName} ${author.profile.lastName}` , value: author._id}));
}
...
export class FormDialog extends React.Component {
	...
	createConversation = () => {
		...
		fetch('/api/conversation/new', {
			...
			body: JSON.stringify({ recipentId: selectedUserId,  composedMessage: message, userId: currentUserId }),
        })
...
return <div>
			<CreateIcon variant='outlined' color='primary' onClick={this.handleClickOpen}>
				Open form dialog
			</CreateIcon>
			<Dialog
				fullWidth
